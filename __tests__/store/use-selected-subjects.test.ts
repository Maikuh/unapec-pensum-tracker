...
const makeSubject = (code: string, prerequisites: string[] = [], credits = 3): Subject => ({
  code,
  name: `Subject ${code}`,
  credits,
  prerequisites,
...
describe("initPensum", () => {
  it("creates an empty array for a new pensumCode", () => {
    ...
    expect(useSelectedSubjectsStore.getState().selectedSubjects[PENSUM]).toEqual([]);
  });
  ...
  it("does not overwrite existing selection", () => {
    ...
    useSelectedSubjectsStore.getState().initPensum(PENSUM);
    expect(useSelectedSubjectsStore.getState().selectedSubjects[PENSUM]).toEqual(existing);
  });
});
...
describe("selectSubject", () => {
  ...
  it("adds a subject when not yet selected", () => {
    ...
    useSelectedSubjectsStore.getState().selectSubject(PENSUM, subject, [subject]);
    expect(useSelectedSubjectsStore.getState().selectedSubjects[PENSUM]).toContainEqual(subject);
  });

  it("removes a subject when already selected", () => {
    const subject = makeSubject("A");
    useSelectedSubjectsStore.setState({ selectedSubjects: { [PENSUM]: [subject] } });
    useSelectedSubjectsStore.getState().selectSubject(PENSUM, subject, [subject]);
    expect(useSelectedSubjectsStore.getState().selectedSubjects[PENSUM]).toHaveLength(0);
  });
  ...
  it("cascade-removes dependent subjects when deselecting", () => {
    ...
    useSelectedSubjectsStore.setState({ selectedSubjects: { [PENSUM]: [a, b, c] } });
...
describe("bulkSelect", () => {
  ...
  it("adds all selectable subjects when status is unchecked", () => {
    useSelectedSubjectsStore.getState().bulkSelect(PENSUM, subjects, 3, "unchecked", 0, 100);
    expect(useSelectedSubjectsStore.getState().selectedSubjects[PENSUM]).toHaveLength(3);
  });

  it("removes all subjects when status is checked", () => {
    useSelectedSubjectsStore.setState({ selectedSubjects: { [PENSUM]: subjects } });
    useSelectedSubjectsStore.getState().bulkSelect(PENSUM, subjects, 3, "checked", 0, 100);
    expect(useSelectedSubjectsStore.getState().selectedSubjects[PENSUM]).toHaveLength(0);
  });

  it("does nothing when status is disabled", () => {
    useSelectedSubjectsStore.getState().bulkSelect(PENSUM, subjects, 3, "disabled", 0, 100);
    expect(useSelectedSubjectsStore.getState().selectedSubjects[PENSUM]).toHaveLength(0);
  });
});
...
describe("importFromFile", () => {
  it("replaces entire selectedSubjects state", () => {
    ...
    expect(useSelectedSubjectsStore.getState().selectedSubjects).toEqual(newData);
  });
});
