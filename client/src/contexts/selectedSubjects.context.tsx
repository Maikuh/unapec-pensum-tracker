...
type SelectedSubjectsAction =
  ActionMap<SelectedSubjectsPayload>[keyof ActionMap<SelectedSubjectsPayload>];
    ...
    case "bulk-select": {
      ...
      if (checkboxStatus === "unchecked") {
        ...
        if (newSelectedSubjects.length < periodSubjectsCount) {
          ...
          newSelectedSubjects.forEach((newSelectedSubject: Subject) => {
...
function SelectedSubjectsProvider({ children }: { children: React.ReactNode }) {
  const defaultSelectedSubjects =
    getSelectedSubjectsInLocalStorage(pensumsJson);
